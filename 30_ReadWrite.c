...
struct s {
 ...
 int main(){
	...
	FILE *fptr;
	int i;
	fptr=fopen("E:\\Cprograms\\MasroorShah_107_C_Programs_Repository\\030_ReadWriteFile.txt","wb"); //Opening the file from the location.
	for (i=0;i<1;++i) {
		fflush(stdin);
		printf("Enter Text: ");
	...
	fclose(fptr);
	fptr=fopen("E:\\Cprograms\\MasroorShah_107_C_Programs_Repository\\030_ReadWriteFile.txt","rb");
